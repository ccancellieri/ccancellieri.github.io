...
Single-page portfolio: dark theme, responsive, zero build step. Self-contained HTML with inline CSS/JS.

**Live:** https://ccancellieri.github.io

## Quick Start (Dev)

```bash
# Clone and open in VS Code
git clone git@github.com:ccancellieri/ccancellieri.github.io.git
cd ccancellieri.github.io
code .

# VS Code will prompt to install recommended extensions (Live Server, Prettier, etc.)
# Then: right-click index.html → "Open with Live Server" (or Alt+L Alt+O)

# Or without VS Code:
make serve          # Python HTTP server on port 5500
make stats          # Show file stats (sections, cards, links)
make validate       # HTML validation (needs html5validator)
make lighthouse     # Lighthouse audit (needs Chrome CLI)
...
```
.
├── index.html          # The entire site (single file, ~76KB)
├── README.md           # This file
├── REFERENCES.md       # All URLs, project links, design tokens, CDN deps
├── Makefile            # Dev commands: serve, deploy, validate, minify, stats
├── .editorconfig       # Editor formatting rules
├── .gitignore          # Files to exclude from git
└── .vscode/
    ├── settings.json   # VS Code workspace settings (Live Server, Prettier)
    └── extensions.json # Recommended extensions
```

## How to Edit

The site is a single `index.html`. Everything is inline — no build step, no npm, no framework.

**To change content:** search for the text you want to change. The HTML is well-commented with section markers like `<!-- ===== HERO SECTION ===== -->`.

**To change colors:** modify the CSS variables in `:root` at the top of the `<style>` block. See `REFERENCES.md` for the full token list.

**To add a project card:** copy an existing `<div class="project-card">` block and modify the content. The grid auto-fits.

**To change fonts:** swap the Google Fonts `<link>` in `<head>` and update `font-family` in the `body` CSS rule.

## References

See [REFERENCES.md](REFERENCES.md) for all URLs, project links, GitHub repos, CDN dependencies, and design tokens used in this site.
